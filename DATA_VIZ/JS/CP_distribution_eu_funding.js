...
d3.csv("../DATA_VIZ/TEST_CSV/AT_cp_03.csv", function (d) {
...
.then(function (data) {
    ...
    const svg = d3.select("div.eu_funding")
        .append("svg")
        .attr("width", width)
        .attr("height", height);
    ...
    // Same code as the previous but it removes the gaps between the bars by overlaying the bars
    // svg.append("g")
    //     .selectAll("rect")
    //     .data(data.reverse())
    //     .join("rect")
    //     .attr("fill", d => colour(d.field_transport_modes))
    //     .attr("x", x(0))     // We reverse the data and each bar start from 0 and each bar is on top of the other
    //     .attr("y", margin.top)
    //     .attr("width", d => x(d.share + d.starting) - x(0))    // Need to do it like this to get the width right with the margins
    //     .attr("height", height - margin.top - margin.bottom);
